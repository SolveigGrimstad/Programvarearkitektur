...
public class GameOverView extends BaseView{
    ...
    public void show() {
        ...
        table.row();
        table.add(gameOver).center().padLeft(50);
        table.row();

        //adds and expends the gameover label to the size of the screen
        table.add(username);
        table.add(usernameField).center().padTop(30);
        table.add(okBtn).expandX();
        ...
        if (checkGameStatus == "ExistingGame"){
            ...
            table.add(new Label("New gamepin: " + dataholder.getGamePin(), skin, "big"));
            table.row().pad(20,0,0,10);
            table.add(menuBtn).expandX();
            table.removeActor(okBtn);
    }
